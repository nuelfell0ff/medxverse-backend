...
function requirementMatches(requirements, bed) {
    const reasons = [];
    const warnings = [];
    ...
    if (requirements.acuityLevel) {
        if (!bed.supportedAcuityLevels.includes(Number(requirements.acuityLevel)))
    ...
    if (requirements.bedType && bed.bedType !== requirements.bedType.toUpperCase())
        return { compatible: false, reasons, warnings };
    ...
    if (requirements.department)
        reasons.push(`Requested department: ${requirements.department}`);
    if (requirements.acuityLevel && Number(requirements.acuityLevel) <= 2)
        warnings.push('High-acuity patient: confirm clinical readiness before assignment.');
    return { compatible: true, reasons, warnings };
}
export class BedMatchingService {
    async suggest(input, limit = 10) {
        ...
        for (const bed of beds) {
            ...
            if (match.compatible && (!requirements.department || ward.department?.toLowerCase() === requirements.department.toLowerCase())) {
                let score = 100;
                if (requirements.department && ward.department?.toLowerCase() === requirements.department.toLowerCase())
                    score += 40;
                if (requirements.bedType && bed.bedType === requirements.bedType.toUpperCase())
                    score += 35;
                if (requirements.acuityLevel && bed.supportedAcuityLevels.includes(Number(requirements.acuityLevel)))
                    score += 30;
                exact.push({ bed: bed, ward: ward, score, reasons: match.reasons, warnings: match.warnings });
                continue;
            }
            ...
            const warnings = ['No fully matching bed was available; operator review required.'];
            let score = 45;
            if (requirements.department) {
                if (ward.department?.toLowerCase() === requirements.department.toLowerCase()) {
                    score += 25;
                    reasons.push('Same department');
                }
                else
                    warnings.push(`Different department: ${ward.department || 'unspecified'}`);
            }
            if (requirements.bedType) {
                if (bed.bedType === requirements.bedType.toUpperCase()) {
                    score += 20;
                    reasons.push('Requested bed type');
                }
                else
                    warnings.push(`Bed type is ${bed.bedType}, requested ${requirements.bedType.toUpperCase()}`);
            }
            if (requirements.acuityLevel) {
                if (bed.supportedAcuityLevels.includes(Number(requirements.acuityLevel))) {
                    score += 20;
                    ...
                    warnings.push(`Does not list acuity ${requirements.acuityLevel} support`);
            }
            const caps = bed.capabilities instanceof Map ? Object.fromEntries(bed.capabilities) : (bed.capabilities || {});
            for (const [key, label] of [['isolation', 'Isolation'], ['negativePressure', 'Negative-pressure'], ['oxygen', 'Oxygen'], ['cardiacMonitor', 'Cardiac monitoring'], ['pediatric', 'Pediatric'], ['bariatric', 'Bariatric'], ['mentalHealthSafeSpace', 'Mental-health safe space']]) {
                if (requirements[key]) {
                    if (caps[key] === true) {
                        score += 8;
                        reasons.push(`${label} capability`);
                        ...
                        warnings.push(`Missing ${label} capability`);
                }
            }
            if (requirements.gender && bed.genderRestriction && bed.genderRestriction.toUpperCase() !== requirements.gender.toUpperCase())
                warnings.push(`Gender restriction: ${bed.genderRestriction}`);
            fallback.push({ bed: bed, ward: ward, score, reasons, warnings });
        }
