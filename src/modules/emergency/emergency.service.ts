...
export class EmergencyService {
  ...
  public async createTriage(visitId: string, hospitalId: string, actorId: string, input: CreateTriageInput) {
    ...
    visit.currentAcuityLevel = input.acuityLevel;
  ...
  public async updateStatus(visitId: string, hospitalId: string, actorId: string, input: UpdateEDStatusInput) {
    ...
    if (visit.status !== input.status) {
      this.appendStatusTransition(visit, input.status, actorId, input.reason);
    }

    if (TERMINAL_STATUSES.includes(input.status)) visit.closedAt = new Date();
    await visit.save();
  ...
  public async recordDisposition(
    ...
    if (workflow) {
      visit.downstreamWorkflows.push({ workflow, status: WorkflowStatus.TRIGGERED, triggeredAt: new Date(), referenceId: record._id.toString() });
    }
  ...
  private async publishTriageToEhr(visit: IEDVisitDocument, triage: any, actorId: string) {
    ...
    await publishEhrResource({
      ...
      resourceId: triage._id.toString(),
      status: 'FINAL',
      department: 'Emergency Department',
      resource: {
        resourceType: 'Observation',
