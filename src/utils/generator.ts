 ...
 */
export const generateOPDVisitNumber = (): string => {
  const date = new Date();
 ...
 */
export const generatePreAuthCode = (): string => {
  const year = new Date().getFullYear();
  const random = Math.floor(10000 + Math.random() * 90000);
 ...
 */
export const generateClaimRef = (): string => {
  const date = new Date();
 ...
 */
export const generateStaffCode = (prefix: string = 'STAFF'): string => {
  const random = Math.floor(1000 + Math.random() * 9000);
