...
export class OccupancyForecastService {
  public async generate(hospitalId: string, horizonDays = 7, wardId?: string) {
    ...
    for (const ward of wards) {
      ...
      const admissionsByDay = new Map<string, number>();
      const dischargesByDay = new Map<string, number>();
      ...
      for (let day = 1; day <= horizon; day += 1) {
        ...
        await OccupancyForecastModel.findOneAndUpdate(
          { hospitalId: hospitalObjectId, wardId: ward._id, forecastDate: date, horizonDays: horizon },
          { $set: forecast },
