...
import {
  ...
  BedStatus,
  BedStatusEventType,
  IBedAssignmentDocument,
  IBedDocument,
  IBedStatusEventDocument,
  IOccupancyForecastDocument,
...
const BedAssignmentSchema = new Schema<IBedAssignmentDocument>(
  {
    hospitalId: { type: Schema.Types.ObjectId, ref: 'Account', required: true, index: true },
...
BedAssignmentSchema.index(
  ...
  { unique: true, partialFilterExpression: { status: { $in: [AssignmentStatus.RESERVED, AssignmentStatus.ACTIVE] } } },
);

const BedStatusEventSchema = new Schema<IBedStatusEventDocument>(
  {
    hospitalId: { type: Schema.Types.ObjectId, ref: 'Account', required: true, index: true },
    bedId: { type: Schema.Types.ObjectId, ref: 'Bed', required: true, index: true },
    wardId: { type: Schema.Types.ObjectId, ref: 'Ward', required: true, index: true },
...
const TransferRequestSchema = new Schema<ITransferRequestDocument>(
  {
    hospitalId: { type: Schema.Types.ObjectId, ref: 'Account', required: true, index: true },
...
const OccupancyForecastSchema = new Schema<IOccupancyForecastDocument>(
  {
    hospitalId: { type: Schema.Types.ObjectId, ref: 'Account', required: true, index: true },
    wardId: { type: Schema.Types.ObjectId, ref: 'Ward', required: true, index: true },
...
export const WardModel = mongoose.models.Ward || model<IWardDocument>('Ward', WardSchema);
export const BedModel = mongoose.models.Bed || model<IBedDocument>('Bed', BedSchema);
export const BedAssignmentModel =
  mongoose.models.BedAssignment || model<IBedAssignmentDocument>('BedAssignment', BedAssignmentSchema);
export const BedStatusEventModel =
  mongoose.models.BedStatusEvent || model<IBedStatusEventDocument>('BedStatusEvent', BedStatusEventSchema);
export const TransferRequestModel =
  mongoose.models.TransferRequest || model<ITransferRequestDocument>('TransferRequest', TransferRequestSchema);
export const OccupancyForecastModel =
  mongoose.models.OccupancyForecast || model<IOccupancyForecastDocument>('OccupancyForecast', OccupancyForecastSchema);
