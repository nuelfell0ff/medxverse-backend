import { emergencyEvents } from '../emergency/emergency.events.js';
import { bedWardService } from './bed-ward.service.js';
import { AdmissionRequestSource } from './bed-ward.types.js';
let initialized = false;
export function initializeBedWardIntegrations() {
    if (initialized)
        return;
    initialized = true;
    emergencyEvents.on('downstream.workflow', async (event) => {
        if (event.workflow !== 'BED_MANAGEMENT' || !event.patientId)
            return;
        try {
