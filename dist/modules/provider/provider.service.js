...
const cleanList = (value) => {
    if (!Array.isArray(value))
        return undefined;
    return [...new Set(value.map(clean).filter((item) => Boolean(item)))];
};
...
const normaliseAccreditation = (input, fallback) => {
    ...
    if (result.issuedAt && result.expiresAt && result.expiresAt < result.issuedAt)
        throw error('Accreditation expiry cannot be before issue date');
    return result;
};
const normaliseContract = (input, fallback) => ({
...
export class HMOProviderService {
    ...
    async getProviders(hmoId, filters) {
        ...
        if (filters.accreditationStatus)
            query['accreditation.status'] = filters.accreditationStatus;
        if (filters.network)
            query.networkIds = filters.network;
    ...
    async updateProvider(id, hmoId, input) {
        ...
        if (input.licenseNumber !== undefined)
            provider.licenseNumber = clean(input.licenseNumber)?.toUpperCase();
        if (input.taxId !== undefined)
            provider.taxId = clean(input.taxId)?.toUpperCase();
        if (input.specialty !== undefined)
            provider.specialty = clean(input.specialty);
        if (input.phone !== undefined)
            provider.phone = clean(input.phone);
        if (input.email !== undefined)
            provider.email = clean(input.email)?.toLowerCase();
        if (input.website !== undefined)
            provider.website = clean(input.website);
        if (input.address !== undefined)
            provider.address = input.address;
        if (input.primaryContact !== undefined)
            provider.primaryContact = input.primaryContact;
        if (input.services !== undefined)
            provider.services = cleanList(input.services) ?? [];
        if (input.networkIds !== undefined)
            provider.networkIds = cleanList(input.networkIds) ?? [];
        if (input.tariffIds !== undefined)
        ...
        if (input.notes !== undefined)
            provider.notes = clean(input.notes);
        return provider.save();
    }
    async updateProviderStatus(id, hmoId, input) {
        ...
        if (input.reason)
            provider.notes = provider.notes ? `${provider.notes}\nStatus: ${input.reason}` : `Status: ${input.reason}`;
        return provider.save();
    }
    async updateAccreditation(id, hmoId, input) {
        const provider = await HMOProviderModel.findOne({ _id: objectId(id, 'provider id'), hmoId: objectId(hmoId, 'hmoId') });
        if (!provider)
            return null;
    ...
    async getStats(hmoId) {
        ...
        const stats = { total: 0, pending: 0, active: 0, inactive: 0, suspended: 0, expired: 0, archived: 0, accredited, expiringAccreditations: expiring };
