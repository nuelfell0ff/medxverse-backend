...
export function attachPharmacyWebSocket(server: HttpServer) {
  const wss = new WebSocketServer({ noServer: true });

  server.on('upgrade', (req, socket, head) => {
    const url = new URL(req.url || '/', `http://${req.headers.host || 'localhost'}`);
    if (url.pathname !== '/ws/pharmacy') return;
    ...
    try {
      ...
      wss.handleUpgrade(req, socket, head, ws => {
        (ws as any).hospitalId = String(hospitalId);
  ...
  const broadcast = (type: string, payload: any) => {
    wss.clients.forEach(client => {
      if (client.readyState === WebSocket.OPEN && String((client as any).hospitalId) === String(payload?.hospitalId)) {
