...
export class ClaimsService {
  public async createClaim(
    ...
    if (!input.claimNumber?.trim()) throw new Error('Claim number is required');
    ...
    const formattedItems: IClaimItem[] = input.items.map((item, index) => {
    ...
    const totalClaimedAmount = normalizeMoney(
      formattedItems.reduce((sum, item) => sum + item.claimedAmount, 0)
    );
  ...
  public async getClaims(
    ...
    if (filters.memberId) query.memberId = toObjectId(filters.memberId, 'member ID');
    if (filters.providerId) query.providerId = toObjectId(filters.providerId, 'provider ID');
    ...
    if (filters.startDate || filters.endDate) {
      ...
      if (filters.endDate) {
        ...
        if (Number.isNaN(date.getTime())) throw new Error('Invalid end date');
        // Make date-only end dates inclusive through the end of that day.
  ...
  public async updateClaimStatus(
    ...
    if (!nextStatus || !allowedTransitions[claim.status].includes(nextStatus)) {
      throw new Error(`Invalid claim status transition: ${claim.status} -> ${nextStatus}`);
    }

    const adjudicatorId = toObjectId(userId, 'user ID');

    if (nextStatus === 'REJECTED' && !input.rejectionReason?.trim()) {
    ...
    if (input.approvedItems) {
      ...
      for (const approved of input.approvedItems) {
        ...
        if (amount > claimed) {
          throw new Error(
            `Approved amount for item ${approved.itemIndex + 1} cannot exceed claimed amount`
          );
        }
      ...
      claim.totalApprovedAmount = normalizeMoney(totalApproved);
    ...
    if (input.rejectionReason?.trim()) claim.rejectionReason = input.rejectionReason.trim();
    if (input.notes !== undefined) claim.notes = input.notes?.trim() || undefined;
