...
function normaliseRules(rules: IBenefitRule[] | undefined): IBenefitRule[] {
  ...
  return rules.map((rule) => {
    ...
    if (seen.has(rule.category)) {
      throw Object.assign(
        new Error(`Duplicate benefit rule category: ${rule.category}`),
        { statusCode: 400 }
      );
    }
    ...
    for (const field of ['annualLimit', 'perVisitLimit', 'copayAmount'] as const) {
      const value = rule[field];
      if (value != null && (!Number.isFinite(value) || value < 0)) {
        throw Object.assign(
          new Error(`${field} must be a non-negative number for ${rule.category}`),
          { statusCode: 400 }
        );
      }
    }
...
export class BenefitsService {
  public async createPackage(
    ...
    if (!code) throw Object.assign(new Error('Benefit package code is required'), { statusCode: 400 });
    if (!name) throw Object.assign(new Error('Benefit package name is required'), { statusCode: 400 });
    ...
    try {
      ...
      if ((error as { code?: number }).code === 11000) {
        throw Object.assign(
          new Error(`A benefit package with code "${code}" already exists`),
          { statusCode: 409 }
        );
      }
      throw error;
  ...
  public async getPackages(
    ...
    const query: Record<string, unknown> = { hmoId: ownerId };
  ...
  public async updatePackage(
    ...
    if (input.code !== undefined) {
      ...
      if (!code) throw Object.assign(new Error('Benefit package code cannot be empty'), { statusCode: 400 });
      update.code = code.toUpperCase();
    }
    if (input.name !== undefined) {
      const name = cleanString(input.name);
      if (!name) throw Object.assign(new Error('Benefit package name cannot be empty'), { statusCode: 400 });
      update.name = name;
    }
    if (input.description !== undefined) update.description = cleanString(input.description);
    if (input.tier !== undefined) update.tier = cleanString(input.tier);
    ...
    if (input.rules !== undefined) update.rules = normaliseRules(input.rules);
    ...
    try {
      ...
      if ((error as { code?: number }).code === 11000) {
        throw Object.assign(
          new Error(`A benefit package with code "${String(update.code)}" already exists`),
          { statusCode: 409 }
        );
      }
      throw error;
