...
import { StaffCategory, StaffClassification, StaffRole, StaffStatus, } from './staff.types.js';
export class StaffService {
     ...
     * ST-000002
     */
    static async generateStaffId(hospitalId) {
    ...
    static async createStaff(hospitalId, dto) {
        ...
        const staff = await Staff.create({
            ...
            staffId,
            category,
            classification: dto.classification || StaffClassification.GENERAL,
            specialties: dto.specialties || [],
            professionalRegistrations: dto.professionalRegistrations || [],
            qualifications: dto.qualifications || [],
            certifications: dto.certifications || [],
            professionalExperience: dto.professionalExperience || [],
            clinicalPrivileges: dto.clinicalPrivileges || [],
            trainingRecords: dto.trainingRecords || [],
            performanceRecords: dto.performanceRecords || [],
            availability: dto.availability || [],
            onCallAssignments: dto.onCallAssignments || [],
            leaveRecords: dto.leaveRecords || [],
            attendanceRecords: dto.attendanceRecords || [],
     ...
     */
    static async getStaffDashboard(hospitalId) {
        if (!Types.ObjectId.isValid(hospitalId)) {
            throw new Error('Invalid hospital ID');
        }
        const [total, active, inactive, doctors, nurses, alliedHealth, consultants, residents, interns,] = await Promise.all([
     ...
     */
    static async getExpiringCredentials(hospitalId, days = 30) {
        if (!Types.ObjectId.isValid(hospitalId)) {
            throw new Error('Invalid hospital ID');
        }
        if (days < 0) {
