...
ICUAdmissionSchema.index({ hospitalId: 1, bedNumber: 1 }, {
    unique: true,
    partialFilterExpression: {
        status: { $in: [ICUCaseStatus.ADMITTED, ICUCaseStatus.STABILIZED] },
    },
});
ICUAdmissionSchema.index({ hospitalId: 1, patientId: 1 }, {
    unique: true,
    partialFilterExpression: {
        status: { $in: [ICUCaseStatus.ADMITTED, ICUCaseStatus.STABILIZED] },
    },
...
export const ICUAdmissionModel = mongoose.models.ICUAdmission || model('ICUAdmission', ICUAdmissionSchema);
export const DeviceReadingModel = mongoose.models.DeviceReading || model('DeviceReading', DeviceReadingSchema);
export const FlowsheetEntryModel = mongoose.models.FlowsheetEntry || model('FlowsheetEntry', FlowsheetEntrySchema);
export const ICUScoreModel = mongoose.models.ICUScore || model('ICUScore', ICUScoreSchema);
