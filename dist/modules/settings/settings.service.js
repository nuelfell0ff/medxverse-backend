...
export class HMOSettingsService {
    ...
    cleanString(value, field, maxLength) {
    ...
    validateColor(value, field) {
        ...
        if (!/^#(?:[0-9a-fA-F]{3}|[0-9a-fA-F]{6})$/.test(value)) {
            throw new Error(`${field} must be a valid hexadecimal color`);
        }
        return value;
    }
    ...
    validateNumber(value, field, min, max) {
        if (value === undefined)
            return undefined;
    ...
    validateInput(input) {
        ...
        if (input.branding) {
            ...
            if ('organizationName' in input.branding) {
                const value = this.cleanString(input.branding.organizationName, 'organizationName', 150);
                if (!value) {
            ...
            if ('shortName' in input.branding) {
                result.branding.shortName =
                    this.cleanString(input.branding.shortName, 'shortName', 80);
            }
            if ('logoUrl' in input.branding) {
                result.branding.logoUrl =
                    this.cleanString(input.branding.logoUrl, 'logoUrl', 1000);
            }
            if ('primaryColor' in input.branding) {
                result.branding.primaryColor =
                    this.validateColor(this.cleanString(input.branding.primaryColor, 'primaryColor', 20), 'primaryColor');
            }
            if ('secondaryColor' in input.branding) {
                result.branding.secondaryColor =
                    this.validateColor(this.cleanString(input.branding.secondaryColor, 'secondaryColor', 20), 'secondaryColor');
            }
            if ('supportEmail' in input.branding) {
                result.branding.supportEmail =
                    this.validateEmail(this.cleanString(input.branding.supportEmail, 'supportEmail', 254));
            }
            if ('supportPhone' in input.branding) {
                result.branding.supportPhone =
                    this.cleanString(input.branding.supportPhone, 'supportPhone', 40);
            }
        }
        if (input.address) {
            result.address = {};
            ...
            for (const field of addressFields) {
                if (field in input.address) {
                    result.address[field] =
                        this.cleanString(input.address[field], field, 200);
                }
            }
            if ('country' in input.address) {
                const country = this.cleanString(input.address.country, 'country', 100);
                if (!country) {
        ...
        if ('currency' in input) {
            const currency = this.cleanString(input.currency, 'currency', 3);
        ...
        if ('timezone' in input) {
            const timezone = this.cleanString(input.timezone, 'timezone', 100);
            if (!timezone) {
        ...
        if ('dateFormat' in input) {
            const dateFormat = this.cleanString(input.dateFormat, 'dateFormat', 30);
            if (!dateFormat) {
        ...
        if (input.claims) {
            ...
            for (const field of booleanFields) {
                if (field in input.claims) {
                    result.claims[field] =
                        this.validateBoolean(input.claims[field], field);
                }
            }
        }
        if (input.preAuthorization) {
            ...
            if ('enabled' in input.preAuthorization) {
                result.preAuthorization.enabled =
                    this.validateBoolean(input.preAuthorization.enabled, 'enabled');
            }
            if ('defaultValidityDays' in
                input.preAuthorization) {
                result.preAuthorization.defaultValidityDays =
                    this.validateNumber(input.preAuthorization.defaultValidityDays, 'defaultValidityDays', 1, 365);
            }
            if ('requireClinicalNotes' in
                input.preAuthorization) {
                result.preAuthorization.requireClinicalNotes =
                    this.validateBoolean(input.preAuthorization.requireClinicalNotes, 'requireClinicalNotes');
            }
            if ('autoExpire' in input.preAuthorization) {
                result.preAuthorization.autoExpire =
                    this.validateBoolean(input.preAuthorization.autoExpire, 'autoExpire');
            }
        }
            ...
            for (const field of notificationFields) {
                if (field in input.notifications) {
                    result.notifications[field] =
                        this.validateBoolean(input.notifications[field], field);
                }
            }
        }
        if (input.security) {
            result.security = {};
            if ('sessionTimeoutMinutes' in
                input.security) {
                result.security.sessionTimeoutMinutes =
                    this.validateNumber(input.security.sessionTimeoutMinutes, 'sessionTimeoutMinutes', 5, 1440);
            }
            if ('maxLoginAttempts' in input.security) {
                result.security.maxLoginAttempts =
                    this.validateNumber(input.security.maxLoginAttempts, 'maxLoginAttempts', 3, 20);
            }
            if ('requireStrongPasswords' in
                input.security) {
                result.security.requireStrongPasswords =
                    this.validateBoolean(input.security.requireStrongPasswords, 'requireStrongPasswords');
            }
        }
        return result;
    }
    async getSettings(hmoId) {
    ...
    async updateSettings(hmoId, input) {
        const hmoObjectId = this.objectId(hmoId);
        ...
        if (validated.branding) {
            Object.assign(settings.branding, validated.branding);
        }
        if (validated.address) {
            Object.assign(settings.address, validated.address);
        }
        if (validated.claims) {
            Object.assign(settings.claims, validated.claims);
        }
        if (validated.preAuthorization) {
            Object.assign(settings.preAuthorization, validated.preAuthorization);
        }
        if (validated.notifications) {
            Object.assign(settings.notifications, validated.notifications);
        }
        if (validated.security) {
            Object.assign(settings.security, validated.security);
        }
    ...
    async resetSettings(hmoId) {
        const hmoObjectId = this.objectId(hmoId);
        return HMOSettingsModel.findOneAndUpdate({ hmoId: hmoObjectId }, {
...
export const hmoSettingsService = new HMOSettingsService();
