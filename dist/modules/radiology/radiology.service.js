...
export class RadiologyService {
    ...
    async uploadPacsImage(file, orderId, hospitalId, uploadedBy) {
        ...
        if (!allowedMimeTypes.has(file.mimetype.toLowerCase())) {
            throw new Error(`Unsupported radiology image type: ${file.mimetype}. Allowed types are JPEG, PNG, WEBP, GIF, BMP and TIFF.`);
        }
    ...
    async uploadPacsImages(orderId, hospitalId, input, uploadedBy) {
        this.assertObjectId(orderId, 'order ID');
        this.assertObjectId(hospitalId, 'hospital ID');
        if (uploadedBy) {
        ...
        try {
            for (const file of files) {
                const image = await this.uploadPacsImage(file, orderId, hospitalId, uploadedBy);
    ...
    async deletePacsImage(orderId, hospitalId, imageId) {
        this.assertObjectId(orderId, 'order ID');
        this.assertObjectId(hospitalId, 'hospital ID');
        ...
        const image = images.find((item) => String(item._id) === imageId);
        if (!image) {
        ...
        if (order.pacsMetadata) {
            order.pacsMetadata.images = images.filter((item) => String(item._id) !== imageId);
            if (!order.pacsMetadata) {
                throw new Error('PACS metadata could not be initialized.');
            }
            order.pacsMetadata.imageCount =
