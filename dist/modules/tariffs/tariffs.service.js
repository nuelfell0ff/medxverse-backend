...
import { TariffStatus, } from './tariffs.types.js';
...
const normalizeProviderRates = (rates) => {
    if (!rates)
        return [];
    const seen = new Set();
...
export class TariffsService {
    async createTariff(input) {
        if (!input.code?.trim())
            throw new Error('Tariff code is required');
        if (!input.name?.trim())
            throw new Error('Tariff name is required');
        if (!input.category)
            throw new Error('Tariff category is required');
    ...
    async getTariffs(hmoId, filters) {
    ...
    async updateTariff(id, hmoId, input) {
        ...
        if (input.code !== undefined) {
            if (!input.code.trim())
                throw new Error('Tariff code cannot be empty');
            tariff.code = input.code.trim().toUpperCase();
        }
        if (input.name !== undefined) {
            if (!input.name.trim())
                throw new Error('Tariff name cannot be empty');
        ...
        if (input.category !== undefined)
            tariff.category = input.category;
    ...
    async setStatus(id, hmoId, status) {
    ...
    async quote(hmoId, input) {
        ...
        if (input.providerId) {
            ...
            const providerRate = tariff.providerRates.find((rate) => rate.providerId.toString() === providerId.toString());
            if (providerRate)
