...
export class ReportsService {
    async generateReport(hmoId, userId, input) {
    ...
    async getReportHistory(hmoId, filters) {
    ...
    async aggregateClaimsSummary(hmoId, params) {
        const matchQuery = {
            hmoId: new Types.ObjectId(hmoId),
        };
        ...
        if (params.startDate || params.endDate) {
            ...
            if (params.startDate) {
                matchQuery.createdAt.$gte = new Date(params.startDate);
            }
            if (params.endDate) {
                matchQuery.createdAt.$lte = new Date(params.endDate);
            }
        }
    ...
    async aggregateFinancialOverview(hmoId, params) {
        const matchQuery = {
            hmoId: new Types.ObjectId(hmoId),
        };
        if (params.startDate || params.endDate) {
