...
export class ConsultationService {
    ...
    static async getConsultations(hospitalId, query) {
    ...
    static async getConsultationById(hospitalId, consultationId) {
        ...
        if (!consultation) {
            const error = new Error('Consultation record not found.');
            error.statusCode = 404;
            throw error;
        }
        return consultation;
    }
    static async updateConsultation(hospitalId, consultationId, dto) {
        ...
        if (dto.physicalExamination !== undefined)
            consultation.physicalExamination = dto.physicalExamination;
        if (dto.diagnoses !== undefined)
            consultation.diagnoses = dto.diagnoses;
        if (dto.treatmentPlan !== undefined)
            consultation.treatmentPlan = dto.treatmentPlan;
        if (dto.prescriptions !== undefined)
            consultation.prescriptions = dto.prescriptions;
        if (dto.labOrders !== undefined)
            consultation.labOrders = dto.labOrders;
        if (dto.followUpDate !== undefined)
            consultation.followUpDate = new Date(dto.followUpDate);
