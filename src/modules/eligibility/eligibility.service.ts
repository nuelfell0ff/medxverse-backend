...
import {
  BenefitEvaluation,
  EligibilityCheckQuery,
  EligibilityDecision,
  EligibilityDecisionResult,
  EligibilityServiceCategory,
  EligibilityStats,
  IEligibilityCheck,
  PaginatedEligibilityChecksResult,
  VerifyEligibilityInput,
} from './eligibility.types.js';

type ServiceError = Error & { statusCode: number };
const fail = (message: string, statusCode = 400): ServiceError => Object.assign(new Error(message), { statusCode });
const objectId = (value: string, field: string): Types.ObjectId => {
  if (!value || !Types.ObjectId.isValid(value)) throw fail(`Invalid ${field}`);
  return new Types.ObjectId(value);
};
const parseDate = (value: string | Date | undefined, field: string): Date => {
  const date = value instanceof Date ? new Date(value.getTime()) : value ? new Date(value) : new Date();
  if (Number.isNaN(date.getTime())) throw fail(`Invalid ${field}`);
  return date;
};
const escapeRegex = (value: string) => value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');

const claimCategoryToBenefit = (category: string): EligibilityServiceCategory => {
  switch (category) {
...
export class EligibilityService {
  public async verify(hmoId: string, input: VerifyEligibilityInput): Promise<EligibilityDecisionResult> {
    ...
    const enrollee = await EnrolleeModel.findOne({ _id: memberId, hmoId: tenantId })
      ...
      .exec();
    if (!enrollee) throw fail('Enrollee not found for this HMO', 404);

    const plan = enrollee.get('benefitPlanId') as any;
    if (!plan?._id) throw fail('The enrollee does not have a valid benefit plan', 422);
    ...
    if (!memberActive) reasons.push(`Membership status is ${enrollee.get('status')}.`);
    if (!withinCoverage) reasons.push('The requested service date is outside the member coverage period.');
    if (!planActive) reasons.push(`Benefit plan status is ${plan.status}.`);

    let providerResult: EligibilityDecisionResult['provider'];
    if (input.providerId) {
      const provider = await HMOProviderModel.findOne({ _id: objectId(input.providerId, 'provider ID'), hmoId: tenantId }).lean().exec();
      if (!provider) reasons.push('Provider is not registered with this HMO.');
      else {
        ...
        if (provider.status !== 'ACTIVE') reasons.push(`Provider status is ${provider.status}.`);
        if (provider.accreditation?.status !== 'VERIFIED') reasons.push('Provider accreditation is not verified.');
        if (input.networkId && !networkMatched) reasons.push('Provider is not participating in the requested network.');
      }
    }
    ...
    const annualUsed = claims.reduce((sum, claim: any) => sum + Number(claim.totalApprovedAmount || 0), 0);
    const annualRemaining = typeof plan.annualMaxBenefit === 'number'
      ? Math.max(0, plan.annualMaxBenefit - annualUsed)
      : undefined;

    const rule = (plan.rules || []).find((item: any) => item.category === input.serviceCategory);
    const categoryUsed = claims.reduce((sum, claim: any) => {
      return sum + (claim.items || []).reduce((itemSum: number, item: any) => {
        return itemSum + (claimCategoryToBenefit(item.category) === input.serviceCategory ? Number(item.approvedAmount ?? 0) : 0);
      }, 0);
    }, 0);

    const benefit: BenefitEvaluation = {
    ...
    if (!rule) reasons.push('No benefit rule is configured for the requested service category.');
    else if (!rule.isCovered) reasons.push('The requested service category is not covered by the member plan.');

    const amount = Number(input.requestedAmount || 0);
    if (benefit.annualRemaining !== undefined && amount > benefit.annualRemaining) reasons.push('Requested amount exceeds the remaining annual category benefit.');
    if (annualRemaining !== undefined && amount > annualRemaining) reasons.push('Requested amount exceeds the remaining annual maximum benefit.');
    ...
    const decision: EligibilityDecision = !eligibleCore ? 'INELIGIBLE' : partial ? 'PARTIAL' : 'ELIGIBLE';
    if (partial) reasons.push('Benefit is covered, but pre-authorization is required before service delivery.');
  ...
  public async getChecks(hmoId: string, filters: EligibilityCheckQuery): Promise<PaginatedEligibilityChecksResult> {
    const tenantId = objectId(hmoId, 'HMO ID');
    ...
    if (filters.search?.trim()) query.$or = [{ serviceCode: new RegExp(escapeRegex(filters.search.trim()), 'i') }, { serviceDescription: new RegExp(escapeRegex(filters.search.trim()), 'i') }];
  ...
  public async getCheckById(id: string, hmoId: string): Promise<IEligibilityCheck | null> {
  ...
  public async getMemberChecks(memberId: string, hmoId: string): Promise<IEligibilityCheck[]> {
  ...
  public async getStats(hmoId: string): Promise<EligibilityStats> {
    const tenantId = objectId(hmoId, 'HMO ID');
