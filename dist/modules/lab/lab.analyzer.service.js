...
import { AnalyzerProtocol, EntryMethod } from './lab.types.js';
import { LabService } from './lab.service.js';
const unwrap = (value) => value.replace(/^\u000b|\u001c\r?\n?$/g, '');
export class LabAnalyzerService {
    static parseHL7(message) {
        ...
        for (const segment of segments) {
            ...
            if (fields[0] === 'MSH')
                analyzerName = fields[2] || fields[3] || undefined;
            if (fields[0] === 'OBR')
                accessionNumber = fields[3] || fields[2] || fields[4] || accessionNumber;
            if (fields[0] === 'OBX') {
                ...
                results.push({ parameterName, value, unit, referenceRange, flag: 'NORMAL', entryMethod: EntryMethod.ANALYZER_AUTOMATED, analyzerName, analyzerResultId });
            }
        }
        if (!accessionNumber)
            throw new Error('HL7 message does not contain an accession number.');
        if (!results.length)
            throw new Error('HL7 message does not contain laboratory results.');
        return { accessionNumber, analyzerName, analyzerResultId, results };
    }
    static parseASTM(message) {
        ...
        for (const record of records) {
            ...
            if (record.startsWith('H|'))
                analyzerName = fields[4] || fields[3] || undefined;
            if (record.startsWith('P|'))
                accessionNumber = fields[3] || accessionNumber;
            if (record.startsWith('O|'))
                accessionNumber = fields[2] || accessionNumber;
        ...
        if (!accessionNumber)
            throw new Error('ASTM message does not contain an accession number.');
        if (!results.length)
            throw new Error('ASTM message does not contain laboratory results.');
        return { accessionNumber, analyzerName, analyzerResultId, results };
    }
    static async ingest(hospitalId, userId, protocol, payload) {
        ...
        if (!order)
            throw new Error('Laboratory order not found for analyzer accession number.');
        ...
        if (specimen && specimen.status !== 'PROCESSED')
            await LabService.processSpecimen(hospitalId, order._id.toString(), userId, { barcode: specimen.barcode, notes: 'Analyzer processing initiated.' });
    ...
    static async buildOrderMessage(hospitalId, orderId, protocol) {
