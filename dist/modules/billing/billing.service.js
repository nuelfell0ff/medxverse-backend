...
export class BillingService {
    ...
    async getInvoiceById(invoiceId, hospitalId) {
    ...
    async recordPayment(invoiceId, hospitalId, input) {
        ...
        const newStatus = newBalanceDue === 0 ? InvoiceStatus.PAID : InvoiceStatus.PARTIALLY_PAID;
        return InvoiceModel.findOneAndUpdate({ _id: invoiceId, hospitalId }, {
            $set: {
    ...
    async cancelInvoice(invoiceId, hospitalId, reason) {
        return InvoiceModel.findOneAndUpdate({ _id: invoiceId, hospitalId, status: InvoiceStatus.PENDING }, {
            $set: {
