...
import { BillingInvoiceModel, ProviderSettlementModel, HmoPaymentModel, BillingReconciliationModel, } from './hmo-billing.model.js';
import { InvoiceStatus, InvoiceType, PaymentStatus, PaymentType, ReconciliationStatus, SettlementStatus, SettlementType, } from './hmo-billing.types.js';
...
const dateRange = (from, to) => {
    ...
    const range = {};
    if (from)
        range.$gte = dateValue(from, 'from date');
    if (to)
        range.$lte = dateValue(to, 'to date');
    return range;
};
export class BillingService {
    async createInvoice(hmoId, input) {
        ...
        if (dueDate < issueDate)
            throw new Error('dueDate cannot be earlier than issueDate');
        const lines = input.lines.map((line) => {
            const quantity = Number(line.quantity);
            const unitAmount = money(Number(line.unitAmount), 'Invoice unit amount');
            if (!Number.isFinite(quantity) || quantity <= 0)
                throw new Error('Invoice quantity must be greater than 0');
            return {
    ...
    async listInvoices(hmoId, filters) {
        const { page, limit, skip } = pageParams(filters);
        const query = { hmoId: objectId(hmoId, 'HMO ID') };
        if (filters.status)
        ...
        if (filters.from || filters.to)
            query.issueDate = dateRange(filters.from, filters.to);
    ...
    async updateInvoice(id, hmoId, input) {
        ...
        if (input.dueDate !== undefined)
            invoice.dueDate = dateValue(input.dueDate, 'dueDate');
        if (input.discountAmount !== undefined)
            invoice.discountAmount = money(Number(input.discountAmount), 'Discount amount');
        if (input.taxAmount !== undefined)
            invoice.taxAmount = money(Number(input.taxAmount), 'Tax amount');
        if (input.notes !== undefined)
            invoice.notes = input.notes.trim() || undefined;
    ...
    async setInvoiceStatus(id, hmoId, status) {
    ...
    async createSettlement(hmoId, input) {
        const hmo = objectId(hmoId, 'HMO ID');
        ...
        const lines = input.lines.map((line) => {
            ...
            if (!Number.isFinite(quantity) || quantity <= 0)
                throw new Error('Settlement quantity must be greater than 0');
        ...
        if (lines.some((line) => !line.description))
            throw new Error('Settlement line description is required');
    ...
    async listSettlements(hmoId, filters) {
        const { page, limit, skip } = pageParams(filters);
        const query = { hmoId: objectId(hmoId, 'HMO ID') };
        if (filters.status)
            query.status = filters.status;
        ...
        if (filters.from || filters.to)
            query.periodEnd = dateRange(filters.from, filters.to);
        if (filters.search?.trim()) {
    ...
    async setSettlementStatus(id, hmoId, status, actorId) {
        ...
        if (!settlement)
            return null;
        if (settlement.status === SettlementStatus.PAID && status !== SettlementStatus.PAID)
            throw new Error('A paid settlement cannot move backwards');
    ...
    async createPayment(hmoId, input) {
        const hmo = objectId(hmoId, 'HMO ID');
        ...
        if (input.paymentType === PaymentType.PREMIUM && !invoiceId)
            throw new Error('Premium payments require an invoice ID');
        if (input.paymentType === PaymentType.PROVIDER_SETTLEMENT && !settlementId)
            throw new Error('Provider settlement payments require a settlement ID');
    ...
    async listPayments(hmoId, filters) {
        const { page, limit, skip } = pageParams(filters);
        const query = { hmoId: objectId(hmoId, 'HMO ID') };
    ...
    async applySuccessfulPayment(payment) {
        if (payment.paymentType === PaymentType.PREMIUM && payment.invoiceId) {
            ...
            if (!invoice)
                throw new Error('Linked premium invoice not found');
        ...
        if (payment.paymentType === PaymentType.PROVIDER_SETTLEMENT && payment.settlementId) {
            ...
            if (!settlement)
                throw new Error('Linked provider settlement not found');
    ...
    async reconcilePayment(hmoId, input, actorId) {
    ...
    async quoteTariff(hmoId, tariffId, providerId, quantity = 1, date) {
        ...
        if (providerId) {
            ...
            if (providerRate)
                unitAmount = providerRate.amount;
        }
        return {
            tariffId: tariff._id,
            code: tariff.code,
    ...
    async capitationSettlement(hmoId, input) {
        ...
        if (!Number.isFinite(memberCount) || memberCount <= 0)
            throw new Error('memberCount must be greater than 0');
