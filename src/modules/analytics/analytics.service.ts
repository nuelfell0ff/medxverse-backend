...
const oid = (value: string, field: string) => {
...
const optionalModel = (names: string[]): Model<any> | null => {
  for (const name of names) {
    const registered = mongoose.models[name];

    if (registered) {
      return registered as Model<any>;
    }
  }
...
const requiredModel = (name: string): Model<any> => {
  const registered = mongoose.models[name];

  if (!registered) {
    throw err(
      `${name} model is not registered. Ensure the corresponding HMO module is loaded before analytics.`,
    );
  }
...
export class HMOAnalyticsService {
  ...
  async summary(
    ...
    const activePlans =
      await HealthPlanModel.countDocuments({
        hmoId: owner,
        status: 'ACTIVE',
      });
    ...
    return {
      ...
      claims: {
        ...
        submitted: number(
          statusMap.get('SUBMITTED')?.count,
        ),

        underReview: number(
          statusMap.get('UNDER_REVIEW')?.count,
        ),

        approved: number(
          statusMap.get('APPROVED')?.count,
        ),
        ...
        duplicateRiskCount: number(
          totals.duplicateRisk,
        ),

        monthly: (c.monthly || []).map(
          (x: any) => ({
            month: x._id,
      ...
      authorization: {
        ...
        pending:
          (authMap.get('PENDING') || 0) +
          (authMap.get('NEW_REQUEST') || 0),
        approved:
          authMap.get('APPROVED') || 0,
        declined:
          (authMap.get('DECLINED') || 0) +
          (authMap.get('REJECTED') || 0),
        requestedAmount:
          authTotals.sums.totalRequestedAmount ||
          0,
        approvedAmount:
          authTotals.sums.totalApprovedAmount ||
          0,
      },
      ...
      finance: {
        ...
        settledAmount:
          settlementTotals.sums.netAmount ||
  ...
  async generateReport(
    ...
    if (format === 'CSV') {
      const rows = Object.entries(payload).flatMap(
        ([section, value]) => {
    ...
    const report =
      await AnalyticsReportModel.create({
        hmoId: oid(hmoId, 'HMO ID'),
        reportNumber,
        type: query.type,
        format,
  ...
  async upsertConsent(
    ...
    const doc =
      await AnalyticsConsentModel.findOneAndUpdate(
        {
          hmoId: owner,
          subjectType:
            input.subjectType,
  ...
  async createComplianceReport(
    ...
    const report =
      await ComplianceReportModel.create({
        hmoId: oid(
          hmoId,
          'HMO ID',
        ),
  ...
  async listCompliance(
    ...
    const [
      ...
      ComplianceReportModel.countDocuments(
        filter,
      ),
    ]);
  ...
  async updateComplianceStatus(
    ...
    const doc =
      await ComplianceReportModel.findOneAndUpdate(
        {
          _id: oid(
            id,
...
export const hmoAnalyticsService =
  new HMOAnalyticsService();
