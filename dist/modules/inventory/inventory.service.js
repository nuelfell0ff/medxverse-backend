...
import { SupplierModel, InventoryItemModel, PurchaseOrderModel, EquipmentModel, } from './inventory.model.js';
import { PurchaseOrderStatus, } from './inventory.types.js';
export class InventoryService {
    ...
    async getInventoryItems(hospitalId, query) {
    ...
    async getPurchaseOrders(hospitalId, query) {
        const page = Math.max(1, query.page || 1);
        const limit = Math.min(50, Math.max(1, query.limit || 20));
        const skip = (page - 1) * limit;
    ...
    async getEquipment(hospitalId, query) {
        const page = Math.max(1, query.page || 1);
        const limit = Math.min(50, Math.max(1, query.limit || 20));
        const skip = (page - 1) * limit;
        const filter = { hospitalId };
