...
/**
 * The subset of an ICU admission document required by the device gateway.
 *
 * The explicit lean type is important here because the current Mongoose model
 * typing is broader than the actual result of findOne(). Without it,
 * TypeScript can infer the result as a document-or-array union.
 */
interface ICUAdmissionGatewayRecord {
  _id: Types.ObjectId;
...
export class ICUDeviceGatewayService {
  public normalize(
    input: IngestDeviceReadingInput,
  ): NormalizedDevicePayload {
    ...
    const protocol =
      input.protocol || DeviceProtocol.VENDOR_API;

    const quality =
      input.quality || ReadingQuality.VALID;
  ...
  public async ingest(
    ...
    const admission =
      ...
      }).lean<ICUAdmissionGatewayRecord>();
    ...
    const flowEntries = normalized.measurements.map(
      (measurement) => ({
        hospitalId: new Types.ObjectId(hospitalId),
        admissionId: new Types.ObjectId(input.admissionId),
        patientId: admission.patientId,
  ...
  private categoryFor(
    ...
    if (
      /bp|pressure|heart|pulse|spo2|oxygen|temp|resp|ecg|rhythm/i.test(
        parameter,
      )
    ) {
      return 'VITALS';
...
export const icuDeviceGatewayService =
  new ICUDeviceGatewayService();