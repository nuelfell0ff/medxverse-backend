...
export class LabService {
    ...
    ========================================================= */
    static async generateUniqueAccessionNumber() {
        for (let attempt = 0; attempt < 10; attempt += 1) {
            ...
            const accessionNumber = `LAB-${year}${month}${day}-${timestamp}-${random}`;
            const existingOrder = await LabOrderModel.exists({
                accessionNumber,
            });
            if (!existingOrder) {
        ...
        const error = new Error('Unable to generate a unique laboratory accession number. Please try again.');
    ...
    ========================================================= */
    static getPredictedTatMinutes(priority, isStat) {
        if (isStat ||
    ...
    static async createOrder(hospitalId, requestingUserId, dto) {
        ...
        if (!dto.patientId ||
            ...
            const error = new Error('A valid patient must be selected.');
            error.statusCode = 400;
            throw error;
        }
        ...
        if (!dto.testName?.trim()) {
            const error = new Error('Laboratory test name is required.');
            error.statusCode = 400;
            throw error;
        }
        if (!dto.testCategory) {
            const error = new Error('Laboratory test category is required.');
            error.statusCode = 400;
            throw error;
        }
        if (!dto.sampleType?.trim()) {
        ...
        const duplicateTestDetected = Boolean(existingDuplicate);
        const duplicateTestMessage = existingDuplicate
            ? `A similar active test order already exists: ${existingDuplicate.accessionNumber}`
            : undefined;
        const initialStatus = dto.sampleCollectionScheduledAt
            ? LabOrderStatus.SAMPLE_SCHEDULED
            : LabOrderStatus.PENDING;
        const predictedTatMinutes = this.getPredictedTatMinutes(priority, isStat);
        /*
        ...
        for (let attempt = 0; attempt < 5; attempt += 1) {
            const accessionNumber = await this.generateUniqueAccessionNumber();
            const barcodeUrl = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(accessionNumber)}`;
            try {
                const order = await LabOrderModel.create({
                    ...
                    consultationId: dto.consultationId &&
                        Types.ObjectId.isValid(dto.consultationId)
                    ...
                    sampleCollectionScheduledAt: dto.sampleCollectionScheduledAt
                ...
                const isAccessionDuplicate = mongoError?.code === 11000 &&
                    (mongoError
                        .keyPattern
                        ?.accessionNumber);
                if (isAccessionDuplicate &&
                    attempt < 4) {
                    continue;
                }
                throw error;
        ...
        const error = new Error('Unable to create laboratory order after generating multiple unique accession numbers.');
        error.statusCode = 500;
        throw error;
    }
    /* =========================================================
       GET ORDERS / WORKLIST
    ========================================================= */
    static async getOrders(hospitalId, query) {
        ...
        if (query.patientId &&
            Types.ObjectId.isValid(query.patientId)) {
            filter.patientId =
    ...
    /* =========================================================
       GET SINGLE ORDER
    ========================================================= */
    static async getOrderById(hospitalId, orderId) {
        if (!Types.ObjectId.isValid(orderId)) {
            const error = new Error('Invalid laboratory order ID.');
            error.statusCode = 400;
            throw error;
        }
        ...
        if (!order) {
            const error = new Error('Laboratory order not found.');
    ...
    /* =========================================================
       SAMPLE COLLECTION
    ========================================================= */
    static async collectSample(hospitalId, orderId, phlebotomistId) {
        const order = await this.getOrderById(hospitalId, orderId);
        if ([
            LabOrderStatus.CANCELLED,
            LabOrderStatus.COMPLETED,
        ].includes(order.status)) {
            const error = new Error('Sample cannot be collected for this order.');
            error.statusCode = 400;
            throw error;
        }
        const now = new Date();
    ...
    ========================================================= */
    static async accessionSpecimen(hospitalId, orderId, technicianId, dto) {
        const order = await this.getOrderById(hospitalId, orderId);
        const now = new Date();
        order.status =
        ...
        order.chainOfCustody.push({
            ...
            location: dto.location ||
                'Central Laboratory',
            notes: `Specimen accessioned and routed to ${order.testCategory}.`,
        });
        await order.save();
        return this.populateOrder(order);
    }
    /* =========================================================
       REJECT SAMPLE
    ========================================================= */
    static async rejectSample(hospitalId, orderId, technicianId, dto) {
        const order = await this.getOrderById(hospitalId, orderId);
        const now = new Date();
        order.status =
        ...
        order.rejectionInfo = {
            ...
            recollectionScheduledAt: dto.recollectionScheduledAt
                ? new Date(dto.recollectionScheduledAt)
                : undefined,
        };
        order.chainOfCustody.push({
            timestamp: now,
        ...
        if (dto.requestRecollection) {
            order.chainOfCustody.push({
                ...
                performedBy: new Types.ObjectId(technicianId),
                notes: 'A new specimen collection is required.',
            });
        }
        await order.save();
        return this.populateOrder(order);
    ...
    /* =========================================================
       RECOLLECT SAMPLE
    ========================================================= */
    static async recollectSample(hospitalId, orderId, phlebotomistId) {
        const order = await this.getOrderById(hospitalId, orderId);
        if (order.status !==
            LabOrderStatus.RECOLLECTION_REQUIRED) {
            const error = new Error('This order does not currently require recollection.');
            error.statusCode = 400;
            throw error;
        }
        const now = new Date();
        ...
        order.chainOfCustody.push({
            ...
            performedBy: new Types.ObjectId(phlebotomistId),
            notes: 'Replacement specimen collected successfully.',
        });
        await order.save();
        return this.populateOrder(order);
    }
    /* =========================================================
       RECORD RESULTS
    ========================================================= */
    static async recordResults(hospitalId, orderId, technicianId, dto) {
        const order = await this.getOrderById(hospitalId, orderId);
        const evaluatedResults = dto.results.map((result) => {
            let flag = result.flag ||
                ResultFlag.NORMAL;
            const numericValue = Number(result.value);
            if (result.value &&
                !Number.isNaN(numericValue)) {
                const parameter = result.parameterName.toLowerCase();
                if (parameter.includes('glucose') &&
                    (numericValue >
                        300 ||
                        numericValue <
                            50)) {
        ...
        if (hasCritical) {
            order.criticalResultNotified =
                true;
            order.aiPatternAlerts.push('CRITICAL RESULT ALERT: Critical laboratory value detected. Immediate clinical review is required.');
        }
        if (hasAbnormal) {
            order.aiPatternAlerts.push('ABNORMAL RESULT FLAG: One or more laboratory values require clinical review.');
        }
        if (dto.notes) {
            order.notes =
                dto.notes;
    ...
    /* =========================================================
       VERIFY RESULTS
    ========================================================= */
    static async verifyResults(hospitalId, orderId, verifierId) {
        const order = await this.getOrderById(hospitalId, orderId);
        if (!order.results.length) {
            const error = new Error('Results must be recorded before verification.');
            error.statusCode = 400;
            throw error;
        }
        const now = new Date();
        ...
        order.authorizationHistory.push({
            level: AuthorizationLevel.VERIFIER,
            authorizedBy: new Types.ObjectId(verifierId),
            authorizedAt: now,
            notes: 'Results verified successfully.',
        });
        order.chainOfCustody.push({
            timestamp: now,
            action: 'RESULTS_VERIFIED',
            performedBy: new Types.ObjectId(verifierId),
            notes: 'Results passed verification.',
        });
        await order.save();
        return this.populateOrder(order);
    }
    ...
    ========================================================= */
    static async authorizeResults(hospitalId, orderId, authorizerId) {
        const order = await this.getOrderById(hospitalId, orderId);
        if (order.status !==
            LabOrderStatus.VERIFIED &&
            order.status !==
                LabOrderStatus.RESULTS_RECORDED) {
            const error = new Error('Results must be recorded or verified before authorization.');
            error.statusCode = 400;
            throw error;
        }
        const now = new Date();
        ...
        order.completedAt =
            now;
        order.authorizationHistory.push({
            level: AuthorizationLevel.SENIOR_SCIENTIST,
            authorizedBy: new Types.ObjectId(authorizerId),
            authorizedAt: now,
            notes: 'Results authorized and released.',
        });
        order.chainOfCustody.push({
            timestamp: now,
            action: 'RESULTS_AUTHORIZED_AND_RELEASED',
            performedBy: new Types.ObjectId(authorizerId),
            notes: 'Final laboratory results released.',
        });
        await order.save();
        return this.populateOrder(order);
    }
    /* =========================================================
       AMEND RESULTS
    ========================================================= */
    static async amendResults(hospitalId, orderId, amendedBy, dto) {
        const order = await this.getOrderById(hospitalId, orderId);
        if (!dto.reason?.trim()) {
            const error = new Error('An amendment reason is required.');
            error.statusCode = 400;
            throw error;
        }
        const previousResults = order.results.map((result) => ({
            parameterName: result.parameterName,
            value: result.value,
            unit: result.unit,
            referenceRange: result.referenceRange,
            ageSexSpecificRange: result.ageSexSpecificRange,
            flag: result.flag,
            ...
            repeatReason: result.repeatReason,
        ...
        order.amendmentHistory.push({
            amendedBy: new Types.ObjectId(amendedBy),
            amendedAt: new Date(),
            reason: dto.reason,
            previousResults,
            newResults: dto.results,
        ...
        order.chainOfCustody.push({
            ...
            performedBy: new Types.ObjectId(amendedBy),
    ...
    /* =========================================================
       REPEAT TEST
    ========================================================= */
    static async repeatTest(hospitalId, orderId, technicianId, dto) {
        const order = await this.getOrderById(hospitalId, orderId);
        const now = new Date();
        order.repeatTests.push({
            repeatedAt: now,
            repeatedBy: new Types.ObjectId(technicianId),
            reason: dto.reason,
            parameterNames: dto.parameterNames || [],
