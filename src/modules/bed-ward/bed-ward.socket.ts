...
export function attachBedWardWebSocket(server: HttpServer): void {
  const wss = new WebSocketServer({ noServer: true });

  server.on('upgrade', (request, socket, head) => {
    try {
      const url = new URL(request.url || '/', `http://${request.headers.host || 'localhost'}`);
      if (url.pathname !== '/ws/bed-ward') return;
  ...
  bedWardEvents.on('board.changed', async (event) => {
    const clients = Array.from(wss.clients).filter(
      (client) =>
        (client as WebSocket & { hospitalId?: string }).hospitalId === event.hospitalId &&
