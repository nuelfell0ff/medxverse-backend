...
import { BillingAccountStatus, BillingSourceModule, ChargeStatus, PaymentMethod, PaymentPlanFrequency, PaymentPlanStatus, PaymentStatus, ReconciliationStatus, RefundStatus, deriveBillingServiceCode, normalizeBillingDepartmentName, BILLING_DEPARTMENT_ALIASES, } from './billing.types.js';
/* =========================================================
   HELPERS
========================================================= */
...
const reference = (prefix) => `${prefix}-${Date.now().toString(36).toUpperCase()}-${Math.random()
    .toString(36)
    .slice(2, 7)
    .toUpperCase()}`;
...
const departmentNameMatchers = (value) => {
    ...
    return [...new Set([normalized, ...aliases])].map((name) => new RegExp(`^${escapeRegex(name)}$`, 'i'));
};
const serviceCodeMatchers = (value) => {
    if (!value?.trim())
        return [];
...
/* =========================================================
   BILLING ACCOUNT
========================================================= */
export const createBillingAccount = async (input, userId) => {
...
export const getBillingSummary = async (accountId) => {
    ...
    return {
        ...
        totalPaid,
        totalRefunded,
        outstandingBalance: money(Math.max(0, netCharges - totalPaid + totalRefunded)),
        availableCredit: money(Math.max(0, totalPaid - totalRefunded - netCharges)),
    };
};
export const getBillingAccount = async (id) => {
...
export const getPatientBillingAccount = async (hospitalId, patientId) => {
...
export const getPatientBilling = async (hospitalId, patientId) => {
...
/* =========================================================
   PRICING CATALOGUE
========================================================= */
export const createPricingCatalogueItem = async (input, userId) => {
    const hospitalId = oid(String(input.hospitalId), 'hospital ID');
    if (!input.name?.trim()) {
        throw new Error('Service name is required.');
    }
    const normalizedDepartmentName = normalizeBillingDepartmentName(input.departmentName);
    /*
     ...
     * possible.
     */
    const code = deriveBillingServiceCode(normalizedDepartmentName, input.code);
    if (!code) {
        throw new Error('Service code could not be determined. Provide a service code or a valid department name.');
    }
    const effectiveFrom = validDate(input.effectiveFrom, 'effective from date');
    ...
    if (existing) {
        throw new Error('A pricing catalogue with this plan name already exists for this service and department.');
    }
...
export const getPricingCatalogue = async (hospitalId, query) => {
 ...
 */
export const resolvePrice = async (input) => {
    const hospitalId = oid(String(input.hospitalId), 'hospital ID');
    ...
    if (input.catalogueItemId) {
        ...
        if (input.departmentName?.trim()) {
            const selectedDepartment = normalizeBillingDepartmentName(selected.departmentName);
            const requestedDepartment = normalizeBillingDepartmentName(input.departmentName);
    ...
    if (departmentId || input.departmentName?.trim()) {
        ...
        if (departmentSpecificCandidates.length > 1) {
            throw new Error(`Multiple active pricing plans exist for "${code}" in this department. Select a pricing catalogue explicitly when creating the clinical order.`);
        }
...
export const getPricingCatalogueHistory = async (id) => {
...
export const updatePricingCatalogueItem = async (id, input, userId) => {
    const item = await PricingCatalogueModel.findById(id);
    if (!item) {
        throw new Error('Pricing catalogue item not found.');
    }
    const priceChanged = input.price !== undefined && money(input.price) !== money(item.price);
    const effectiveFromChanged = input.effectiveFrom !== undefined &&
    ...
    if (priceChanged || effectiveFromChanged) {
        item.history.push({
            ...
            reason: 'Previous pricing version archived before catalogue price update.',
        });
        item.version += 1;
    }
    const previousDepartmentName = normalizeBillingDepartmentName(item.departmentName);
    if (input.code !== undefined) {
        const suppliedCode = input.code.trim().toUpperCase();
        if (!suppliedCode) {
            throw new Error('Service code cannot be empty.');
        }
        item.code = deriveBillingServiceCode(input.departmentName !== undefined
            ? input.departmentName
            : item.departmentName, suppliedCode);
    }
         ...
         */
        const autoCodeForPreviousDepartment = previousDepartmentName
    ...
    if (input.effectiveFrom !== undefined) {
        item.effectiveFrom = validDate(input.effectiveFrom, 'effective from date');
    }
 ...
 */
const getAvailableCredit = async (billingAccountId) => {
...
export const createCharge = async (input) => {
    ...
    if (input.sourceModule === BillingSourceModule.MANUAL) {
        if (input.overridePrice !== undefined) {
            throw new Error('overridePrice is only used when overriding a catalogue price.');
        }
        ...
        if (!input.serviceCode?.trim()) {
            throw new Error('serviceCode is required for module-generated billing charges.');
        }
        ...
        if (input.overridePrice !== undefined) {
            if (!input.overrideReason?.trim()) {
                throw new Error('An override reason is required when overriding a catalogue price.');
            }
            overrideApplied = resolvedUnitPrice !== cataloguePrice;
            if (!overrideApplied) {
                throw new Error('The override price must differ from the catalogue price.');
            }
        }
    }
     ...
     */
    const allocation = creditBeforeCharge > 0
        ? await allocatePaymentToCharges(String(account._id), creditBeforeCharge)
...
export const getCharges = async (hospitalId, query) => {
    const page = Math.max(1, Number(query.page) || 1);
    const limit = Math.min(100, Math.max(1, Number(query.limit) || 25));
    const filter = {
        hospitalId: oid(hospitalId, 'hospital ID'),
    ...
    if (query.billingAccountId) {
        filter.billingAccountId = oid(query.billingAccountId, 'billing account ID');
    }
    if (query.category)
        filter.category = query.category;
    if (query.status)
...
/* =========================================================
   PAYMENT ALLOCATION
========================================================= */
const allocatePaymentToCharges = async (billingAccountId, amount) => {
...
export const createPayment = async (input) => {
    ...
    const payment = await PaymentModel.create({
        ...
        reconciliationStatus: input.method === PaymentMethod.CASH
            ? ReconciliationStatus.RECONCILED
            : ReconciliationStatus.UNRECONCILED,
        refundedAmount: 0,
    });
    const allocation = await allocatePaymentToCharges(String(account._id), amount);
    return {
        payment,
        allocation,
    };
};
export const getPayments = async (hospitalId, query) => {
    const page = Math.max(1, Number(query.page) || 1);
    const limit = Math.min(100, Math.max(1, Number(query.limit) || 25));
    const filter = {
        hospitalId: oid(hospitalId, 'hospital ID'),
    ...
    if (query.paymentMethod)
        filter.method = query.paymentMethod;
...
export const reconcilePayment = async (id, input) => {
...
/* =========================================================
   REFUNDS
========================================================= */
export const getRefunds = async (hospitalId, query) => {
    const page = Math.max(1, Number(query.page) || 1);
    const limit = Math.min(100, Math.max(1, Number(query.limit) || 25));
    const filter = {
        hospitalId: oid(hospitalId, 'hospital ID'),
...
export const createRefund = async (input) => {
    ...
    return RefundModel.create({
        ...
        billingAccountId: oid(String(input.billingAccountId), 'billing account ID'),
...
export const decideRefund = async (id, approved, userId, rejectionReason) => {
...
export const completeRefund = async (id) => {
    ...
    const remainingRefundable = money(payment.amount - payment.refundedAmount);
    if (refund.amount > remainingRefundable) {
        throw new Error('Refund exceeds the remaining refundable payment amount.');
    }
    payment.refundedAmount = money(payment.refundedAmount + refund.amount);
    payment.status =
...
/* =========================================================
   PAYMENT PLANS
========================================================= */
export const getPaymentPlans = async (hospitalId, query) => {
    const page = Math.max(1, Number(query.page) || 1);
    const limit = Math.min(100, Math.max(1, Number(query.limit) || 25));
    const filter = {
        hospitalId: oid(hospitalId, 'hospital ID'),
...
export const createPaymentPlan = async (input) => {
    ...
    while (remaining > 0) {
        ...
        if (input.frequency === PaymentPlanFrequency.WEEKLY) {
            ...
            throw new Error('CUSTOM payment plans require a custom scheduling implementation.');
        }
    }
    return PaymentPlanModel.create({
        ...
        startDate: start,
        endDate: validDate(input.endDate, 'end date') ||
