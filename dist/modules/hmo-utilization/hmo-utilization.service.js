...
import { AlertStatus, FraudCaseStatus, FraudEntityType, RiskSeverity, RuleOperator, UtilizationEventType, UtilizationSourceType, } from './hmo-utilization.types.js';
import { FraudAlertModel, FraudCaseModel, FraudRuleModel, UtilizationEventModel, } from './hmo-utilization.model.js';
...
const dateRange = (query) => {
    ...
    if (query.fromDate) {
        ...
        if (Number.isNaN(date.getTime()))
            throw error('Invalid fromDate');
        result.$gte = date;
    }
    if (query.toDate) {
        const date = new Date(query.toDate);
        if (Number.isNaN(date.getTime()))
            throw error('Invalid toDate');
        result.$lte = date;
    }
...
export class HMOUtilizationService {
    ...
    async getUtilizationEvents(hmoId, filters = {}) {
        const owner = objectId(hmoId, 'HMO ID');
        const { page, limit } = pageArgs(filters);
        const query = { hmoId: owner };
        if (filters.memberId)
    ...
    async createRule(hmoId, input) {
        const owner = objectId(hmoId, 'HMO ID');
        const code = clean(input.code)?.toUpperCase();
        const name = clean(input.name);
        if (!code)
            throw error('Rule code is required');
        if (!name)
            throw error('Rule name is required');
        if (!clean(input.category))
            throw error('Rule category is required');
        try {
            ...
            if (err?.code === 11000)
                throw error(`Fraud rule with code "${code}" already exists`, 409);
            throw err;
        }
    }
    async getRules(hmoId) {
        return FraudRuleModel.find({ hmoId: objectId(hmoId, 'HMO ID') })
            .sort({ enabled: -1, createdAt: -1 })
            .lean();
    }
    async updateRule(hmoId, id, input) {
        const update = {};
        if (input.name !== undefined)
            update.name = clean(input.name);
        if (input.description !== undefined)
            update.description = clean(input.description);
        if (input.category !== undefined)
            update.category = clean(input.category);
        if (input.entityType !== undefined)
            update.entityType = input.entityType;
        if (input.severity !== undefined)
            update.severity = input.severity;
        if (input.enabled !== undefined)
            update.enabled = input.enabled;
        if (input.threshold !== undefined)
            update.threshold = Math.max(0, Number(input.threshold) || 0);
        if (input.operator !== undefined)
            update.operator = input.operator;
        if (input.windowDays !== undefined)
            update.windowDays = Math.min(365, Math.max(1, Number(input.windowDays) || 30));
        if (input.action !== undefined)
            update.action = input.action;
        const rule = await FraudRuleModel.findOneAndUpdate({ _id: objectId(id, 'rule ID'), hmoId: objectId(hmoId, 'HMO ID') }, { $set: update }, { new: true, runValidators: true });
        if (!rule)
            throw error('Fraud rule not found', 404);
        return rule;
    }
    async getAlerts(hmoId, filters = {}) {
        const owner = objectId(hmoId, 'HMO ID');
        const { page, limit } = pageArgs(filters);
        const query = { hmoId: owner };
        if (filters.status)
        ...
        if (filters.providerId)
            query.providerId = objectId(filters.providerId, 'provider ID');
        if (filters.ruleId)
            query.ruleId = objectId(filters.ruleId, 'rule ID');
        const [items, total] = await Promise.all([
    ...
    async reviewAlert(hmoId, id, input, reviewerId) {
        if (!Object.values(AlertStatus).includes(input.status))
            throw error('Invalid alert status');
        const alert = await FraudAlertModel.findOne({
            _id: objectId(id, 'alert ID'),
            hmoId: objectId(hmoId, 'HMO ID'),
        });
        if (!alert)
            throw error('Fraud alert not found', 404);
        alert.status = input.status;
        alert.reviewNote = clean(input.note);
        alert.reviewedAt = new Date();
        if (reviewerId && Types.ObjectId.isValid(reviewerId))
            alert.reviewedBy = new Types.ObjectId(reviewerId);
        await alert.save();
        return alert;
    }
    async createCase(hmoId, input) {
        const owner = objectId(hmoId, 'HMO ID');
    ...
    async updateCase(hmoId, id, input) {
        const update = {};
        ...
        if (input.assignedTo !== undefined)
            update.assignedTo = input.assignedTo ? objectId(input.assignedTo, 'assignee ID') : undefined;
        if (input.finding !== undefined)
            update.finding = clean(input.finding);
        if (input.recoveredAmount !== undefined)
            update.recoveredAmount = Math.max(0, Number(input.recoveredAmount) || 0);
        ...
        const item = await FraudCaseModel.findOneAndUpdate({ _id: objectId(id, 'case ID'), hmoId: objectId(hmoId, 'HMO ID') }, { $set: update }, { new: true, runValidators: true });
        if (!item)
            throw error('Fraud case not found', 404);
        return item;
    }
    async getSummary(hmoId) {
        const owner = objectId(hmoId, 'HMO ID');
        const [eventStats, openAlerts, highRiskAlerts, cases, confirmedCases, financials] = await Promise.all([
