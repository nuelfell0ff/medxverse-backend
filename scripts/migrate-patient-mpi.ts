...
if (!MONGO_URI) {
  throw new Error('Set MONGODB_URI or MONGO_URI before running the migration.');
}

const generateMPI = () =>
  `MPI-${crypto.randomBytes(8).toString('hex').toUpperCase()}`;
...
for await (const patient of cursor) {
  if (!patient.universalPatientId) {
    ...
    while (await PatientModel.exists({ universalPatientId: mpi })) {
      mpi = generateMPI();
    }
