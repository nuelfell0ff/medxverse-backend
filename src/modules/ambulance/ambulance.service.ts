...
export class AmbulanceService {
  public async addAmbulance(input: CreateAmbulanceInput): Promise<IAmbulanceDocument> {
  ...
  public async getAmbulances(
    hospitalId: string,
    query: GetAmbulancesQuery
  ): Promise<{ ambulances: IAmbulanceDocument[]; total: number; page: number; totalPages: number }> {
  ...
  ): Promise<IAmbulanceDocument | null> {
  ...
  public async getTripRequests(
    hospitalId: string,
    query: GetTripRequestsQuery
  ): Promise<{ requests: ITripRequestDocument[]; total: number; page: number; totalPages: number }> {
    const page = Math.max(1, query.page || 1);
    const limit = Math.min(50, Math.max(1, query.limit || 20));
    const skip = (page - 1) * limit;
    ...
    if (query.ambulanceId) filter.ambulanceId = query.ambulanceId;
  ...
  public async getTripRequestById(
    requestId: string,
    hospitalId: string
  ): Promise<ITripRequestDocument | null> {
  ...
  public async assignTrip(
    ...
    const trip = await TripRequestModel.findOneAndUpdate(
    ...
    if (trip) {
      await AmbulanceModel.updateOne(
        { _id: ambulanceObjectId, hospitalId },
        { $set: { status: VehicleStatus.DISPATCHED } }
      );
    }
  ...
  public async updateTripStatus(
    ...
    if (input.clinicalNotes) trip.clinicalNotes = input.clinicalNotes;
    if (input.cancellationReason) trip.cancellationReason = input.cancellationReason;
    ...
    if (
      (input.status === TripStatus.COMPLETED || input.status === TripStatus.CANCELLED) &&
      !trip.completionTime
    ) {
      ...
      if (trip.ambulanceId) {
        await AmbulanceModel.updateOne(
          { _id: trip.ambulanceId, hospitalId },
      ...
      await AmbulanceModel.updateOne(
        { _id: trip.ambulanceId, hospitalId },
        { $set: { status: VehicleStatus.EN_ROUTE } }
      );
    }
