...
export class PharmacyInventoryService {
  static async atomicDecrement(
    ...
    if (!before) {
      throw Object.assign(
        new Error('Inventory item is unavailable, expired, or has insufficient stock.'),
        { statusCode: 409 },
      );
    }
    ...
    const updated = await InventoryItemModel.findOneAndUpdate(
      ...
      },
      {
        ...
        $set: {
          isLowStock:
            before.quantityInStock - quantity <= before.reorderLevel,
        },
      },
      {
        new: true,
    ...
    if (!updated) {
      throw Object.assign(
        new Error('Stock changed concurrently. Please rescan and retry.'),
        { statusCode: 409 },
      );
    }
  ...
  static async adjust(
    ...
    type: InventoryTransactionType | undefined,
    performedBy: string,
    reason?: string,
    session?: ClientSession,
  ) {
    ...
    if (!Number.isInteger(quantityChange) || quantityChange === 0) {
      throw Object.assign(
        new Error('Stock adjustment must be a non-zero whole number.'),
        { statusCode: 400 },
      );
    }
    ...
    const allowedTypes = new Set<InventoryTransactionType>([
      InventoryTransactionType.ADJUSTMENT_IN,
      InventoryTransactionType.ADJUSTMENT_OUT,
    ]);
    ...
    if (newQuantity < 0) {
      throw Object.assign(
        new Error('Stock adjustment cannot make inventory negative.'),
        { statusCode: 409 },
      );
    }
    ...
    if (!updated) {
      throw Object.assign(
        new Error(
          'Stock adjustment failed because stock changed concurrently or would become negative.',
        ),
        { statusCode: 409 },
      );
    }
