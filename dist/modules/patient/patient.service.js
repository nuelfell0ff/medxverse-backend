...
export class PatientService {
    ...
    static async importHistoricalEHRResource(hospitalId, dto) {
        if (dto.resourceType === 'Patient')
            this.bad('Patient resources must be migrated through the MPI migration.');
        this.assertObjectId(hospitalId, 'hospital ID');
        this.assertObjectId(dto.patientId, 'patient ID');
        const existing = await EhrEventModel.findOne({ sourceKey: dto.sourceKey }).lean().exec();
        if (existing)
            return { status: 'SKIPPED', resourceId: String(existing.resourceId) };
        await this.assertPatient(hospitalId, dto.patientId);
    ...
    static async rebuildEHRView(hospitalId, patientId) {
        await this.refreshEhrView(hospitalId, patientId);
    }
