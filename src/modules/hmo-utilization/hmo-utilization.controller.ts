...
const getHmoId = (req: Request): string => {
  ...
  if (!value || typeof value !== 'string') {
    const err: any = new Error('HMO ID not found in authentication context.');
    err.statusCode = 400;
    throw err;
  }
...
export class HMOUtilizationController {
  ...
  async events(req: Request, res: Response) {
    try {
      return res.json({
        success: true,
        data: await service.getUtilizationEvents(
          getHmoId(req),
          req.query as any,
        ),
      });
  ...
  async createEvent(req: Request, res: Response) {
    try {
      return res.status(201).json({
        success: true,
        data: await service.createUtilizationEvent(
          getHmoId(req),
          req.body,
        ),
      });
  ...
  async reviewAlert(req: Request, res: Response) {
    try {
      return res.json({
        success: true,
        data: await service.reviewAlert(
          getHmoId(req),
          req.params.id,
          req.body,
          getUserId(req),
