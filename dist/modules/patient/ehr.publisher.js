 ...
 */
export const publishEhrResource = async (input) => {
    const actor = { userId: input.actorId, role: input.role };
    try {
        await PatientService.updateEHRResource(input.hospitalId, actor, input.resourceType, input.resourceId, {
        ...
        if (error?.statusCode !== 404) {
            console.error(`[EHR] Failed to update ${input.resourceType}/${input.resourceId}:`, error instanceof Error ? error.message : error);
            return;
        }
        try {
            ...
            console.error(`[EHR] Failed to create ${input.resourceType}/${input.resourceId}:`, createError instanceof Error ? createError.message : createError);
        }
    }
};
